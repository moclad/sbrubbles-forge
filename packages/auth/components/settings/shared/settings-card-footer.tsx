  ...
  return (
    <CardFooter
      className={cn(
        ...
        variant === 'default' ? 'dark:bg-transparent' : 'border-destructive/30 bg-destructive/10',
        className,
        classNames?.footer
      )}
    >
      {isPending ? (
        <>
          <Skeleton className={cn('my-0.5 h-3 w-48 max-w-full md:h-4 md:w-56', classNames?.skeleton)} />
        ...
        <>
          ...
          {actionLabel && (
            ...
            >
              <span className={cn(!optimistic && isSubmitting && 'opacity-0')}>{actionLabel}</span>
            </Button>
          )}
        </>
      )}
