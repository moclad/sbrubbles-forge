    ...
    return (
      <g>
        ...
        <text
          ...
          x={ex + (cos >= 0 ? 1 : -1) * 12}
          y={ey}
        >{`${payload.label} ${formatCurrency(value, currency, locale)} (${((percent ?? 1) * 100).toFixed(1)}%)`}</text>
      </g>
    );
  };

const PieShape = (currency: string, locale: string) => (props: PieSectorShapeProps) =>
  renderActiveShape(currency, locale)(props as PieSectorDataItem);

export function TripCostSummaryCard({ currency, expenses }: Readonly<TripCostSummaryCardProps>) {
  const t = useI18n();
    ...
    <Card>
      ...
      <CardContent className='space-y-4'>
        <div className='rounded-lg border bg-muted/20 p-3'>
          ...
          <p className='font-semibold text-2xl'>{formatCurrency(totalAmount, currency, locale)}</p>
        </div>
            ...
            <ChartContainer config={chartConfig}>
              <PieChart>
                <Pie
                  ...
                  paddingAngle={4}
                  radius={10}
                  shape={PieShape(currency, locale)}
                />
              </PieChart>
            </ChartContainer>
          </div>
